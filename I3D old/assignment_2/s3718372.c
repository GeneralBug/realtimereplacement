...
float calcDistance(vec3f p1, vec3f p2)
{
	...
	distance = sqrtf(disX*disX + disY*disY + disZ*disZ);
	return distance;
}
...
void keyboardCB (unsigned char key, int x, int y)
{
	...
	case '=':
		...
		g.tesselation *= 2;
		if(g.debug)
			printf("increasing tesselation: %f\n", g.tesselation);
		break;
	case '-':
		if(g.tesselation > 4)
			g.tesselation /= 2;
...
void displayOSD()
{
	...
	glRasterPos2i(10, 60);
	snprintf(buffer, sizeof buffer, "fr (f/s): %6.0f", g.frameRate);
	...
	glColor3f(1.0, 1.0, 0.0);
	glRasterPos2i(10, 20);
	snprintf(buffer, sizeof buffer, "tesselation: %d", g.tesselation);
	for (bufp = buffer; *bufp; bufp++)
	glutBitmapCharacter(GLUT_BITMAP_9_BY_15, *bufp);
